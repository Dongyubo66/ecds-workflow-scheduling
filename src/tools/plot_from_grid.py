...
def main():
    ...
    for inst in insts:
        for metric in [m for m in DEFAULT_METRICS if m in agg.columns]:
            p = plot_metric_vs_w4(agg, inst, metric, outdir)
            if p:
                saved.append(p)
        if args.pareto:
            p = plot_pareto(agg, inst, outdir, x_metric="makespan", y_metric="total_carbon")
            if p:
                saved.append(p)
